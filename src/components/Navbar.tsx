...
export default function Navbar() {
    ...
    return (
        <nav className="bg-white">
            <div className={`flex ${toggle ? `border-b-2` : `shadow-md`} w-full mx-auto px-8 h-20 relative justify-between items-center`}>
            ...
            {toggle && <div className="w-full block sm:hidden h-20 shadow-md transform transition-all items-center bg-white px-5">
                {!loggedIn ?
                    <div className="h-full flex">
                        <button
                            onClick={logOut}
                            className="bg-indigo-600 my-auto rounded-md text-white w-fit px-3 py-1">
                            Logout
                        </button>
                    </div>
                    :
